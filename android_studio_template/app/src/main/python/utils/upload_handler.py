...
class UploadHandler:
    ...
    @staticmethod
    def _upload_base(app):
        """Writable base dir: app.config['UPLOAD_ROOT'] on Android, else app.root_path"""
        return Path(app.config.get('UPLOAD_ROOT') or app.root_path)

    @staticmethod
    def init_app(app):
        """Initialize upload folder on app startup"""
        upload_path = UploadHandler._upload_base(app) / UploadHandler.UPLOAD_FOLDER
    ...
    def save_file(file, patient_id, record_type):
        ...
        # Build full path
        upload_path = UploadHandler._upload_base(current_app) / UploadHandler.UPLOAD_FOLDER
    ...
    def delete_file(photo_path):
        ...
        # Build full path
        full_path = UploadHandler._upload_base(current_app) / photo_path
        
        # Prevent path traversal
        try:
            upload_folder = UploadHandler._upload_base(current_app) / UploadHandler.UPLOAD_FOLDER
