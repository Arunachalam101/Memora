...
# On Android the cwd may be read-only; android_launcher.py creates the real writable dir.
try:
...
@app.route('/static/uploads/memory/<path:filename>')
def uploaded_memory_photo(filename):
    """Serve uploaded photos from the writable UPLOAD_ROOT (Android), if configured."""
    from flask import send_from_directory
    upload_root = app.config.get('UPLOAD_ROOT')
    if not upload_root:
        # Not on Android: fall through to default static handling.
        from flask import abort
        abort(404)
    upload_dir = os.path.join(upload_root, UploadHandler.UPLOAD_FOLDER)
