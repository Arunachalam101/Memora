        ...
        fetch('/api/users/patients', { credentials: 'include' })
            ...
            .then(data => {
                ...
                if (Array.isArray(data)) {
                    ...
                    // Auto-select the first patient so caregivers don't have to
                    // manually reselect before Add/Upload buttons become visible.
                    const firstPatientOption = select.querySelector('option[value]:not([value=""])');
                    if (firstPatientOption) {
                        select.value = firstPatientOption.value;
                        this.selectedPatientId = parseInt(firstPatientOption.value);
                        this.showEditButtons();
                        this.loadData(this.currentTab);
                    }
                }
