/**
 * MEMORA Photo / Name Match Game - Phase 10B
 * Patient sees person's photo and selects correct name from options
 */
...
let gameState = {
    ...
    difficulty: 'medium',
    gameActive: false,
    answers: []  // Track which questions were answered correctly
};
...
function generateQuestions() {
    /**
     * Generate TOTAL_QUESTIONS questions with random people and shuffled answers
     */
    gameState.questions = [];
    ...
    for (let i = 0; i < GAME_CONFIG.TOTAL_QUESTIONS; i++) {
        ...
        const numChoices = Math.min(
            ...
            gameState.people.length  // Can't have more choices than people available
        );
        ...
        // Create answer options with correct answer
...
function displayQuestion() {
    ...
    const photoImg = document.getElementById('question-photo');
    if (question.correctPerson.photo) {
        photoImg.src = question.correctPerson.photo;
    ...
    question.options.forEach((person, index) => {
...
function selectAnswer(personId) {
    ...
    // Disable buttons and show feedback (optional: could add visual feedback here)
